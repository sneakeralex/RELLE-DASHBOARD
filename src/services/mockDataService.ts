...
export const generateMockUsers = (count: number = 100): User[] => {
  ...
  const locations = ['Downtown', 'Uptown', 'Westside', 'Eastside', 'Northside'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.push({
      ...
      preferredLocation: locations[randomNumber(0, locations.length - 1)]
    });
  }
...
export const generateMockOrders = (users: User[], count: number = 300): Order[] => {
  ...
  const locations = ['Downtown', 'Uptown', 'Westside', 'Eastside', 'Northside'];
  const staffNames = ['Emma', 'Olivia', 'Ava', 'Isabella', 'Sophia', 'Mia', 'Charlotte', 'Amelia', 'Harper', 'Evelyn'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const staffId = `staff-${randomNumber(1, 10)}`;
    const staffName = staffNames[randomNumber(0, staffNames.length - 1)];
